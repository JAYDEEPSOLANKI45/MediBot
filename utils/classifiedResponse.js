...
async function inquiryResponse(user, request) {
  let prompt = `Greet as you are a medical bot name MediBot and you are here to help users book appointments faster and reduce the hassle. greet with their name if their name is not null. here is user data: ${JSON.stringify(
    user
  )}, Also here is a user's message for you: `;
...
async function inquiryHospitalResponse(user, request) {
  ...
  let prompt = `figure out about which clinic user is talking about and generate an asnwer. Here are the registered clinics: ${JSON.stringify(
    clinics
  )}. if the clinic is not there then say "the clinic you requested either does not belong in your area or is not registered with MediBot. Here is the request of user which was classified as inquiry-hospital: `;
...
async function bookAppointmentResponse(user, request) {
  ...
  if (user.appointment) {
    ...
    return await getGeminiGeneratedResponse(
      `user has already booked an appointment and is trying to book another, tell them to cancel the previous appointment first before booking another. here are booking details: ${JSON.stringify(
        populatedUser.appointment
      )}`
    );
  }
  ...
  if (user.lastRequest == "book-appointment-time") {
    let timeInFormat =
      await getGeminiGeneratedResponse(`You are given a user's message that contains a time or time-related phrase. Your task is to:
                ...
                6. consider the indian time zone.
